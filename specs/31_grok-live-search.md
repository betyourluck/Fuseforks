...
| **3** | stranded 警告 | protocol を「OpenAI 互換」へ戻す（トグルは真のまま） | 画面に警告 2 行。**送信は通り**、`xai search:` は出ない | 未 |
...
| **6** | 互換運用が壊れていない | イクスを互換のまま普通に会話 | **base URL の警告が出ない**（P2 で直した誤検知の実機確認） | 未 |
| **7** | パッシブ表示 | ザリのテンプレートを開く | 「固有スキル」見出し + `プロンプトキャッシュ [パッシブ]` | 未 |
