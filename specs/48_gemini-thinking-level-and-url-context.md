...
- README 3 言語: グラウンディングの行に「URL 取得」を 1 語（行数は動かない）
- DETAIL 日英: 「Google 検索によるグラウンディング」の直後に「Gemini の思考段階と
  URL context」を新設 + ディレクトリ木の `gemini.rs` の説明。**Spec 05 の
  「参照した URL は渡ってこない」は 3.8 で半分嘘になっていた**（`groundingChunks` は
  返るが redirect URL）— 引用ブロックは触らず、新節で「実 URL は渡ってこない」と
  読み替えを書いた（凍結の文を書き換えるより、何が変わったかを隣に書く）
- `data_contract`: Spec 33 の Gemini 思考の注記が `includeServerSideToolInvocations` を
  「接地を使うときだけ送る」と引き合いに出していた — P0 で invariants を直したのに
  **同じファイルの 400 行上の注記が旧条件のまま**だった（#51 の「隣の節」。
  「接地を使うときだけ」で grep して回収）
- CLAUDE.md: 接地の節の見立てへ続報 + 「Spec の状態」
- **grep 網の外**: ランディングページは固有スキルを列挙していないので嘘にならない /
  Qiita 記事は「モデル登録と固有スキル」に触れている（CLAUDE.md の記録）が、本文を
  この Spec では読み直していない — **未確認**。固有スキルの列挙が載っていれば Gemini の
  行が 1 本足りない形になる（次に記事を触るとき数える）
