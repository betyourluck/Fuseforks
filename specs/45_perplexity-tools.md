...
- 起票: 2026-08-27
- 状態: **rev2 承認 → P0〜P4 完了 = Done**（2026-08-27。**起票から Done まで
  同日**。P4 実機記録は下）。以下は P3 時点の記録:
  **rev2 承認（2026-08-27）→ P0〜P3 完了。残は P4 の実機検収 7 件**。
...
## P4 実機記録（2026-08-27。利用者検証）

- **4 ツールをそれぞれ 1 本だけ ON にして、全部が発火することを観測**
  （検収 1〜4・7 の実体 — finance は実データ + 出典、web は実 URL の出典）。
  **単独 ON の走行が検収 5 の対照を兼ねる** — ON にした 1 本以外の
  `pplx tools:` の分子は 0 のまま（提示していないツールは呼ばれない）
- **検収 6（相乗りの回帰・警告ゼロ）は実機の明示観測なしで閉じた** —
  機械側の網（`providerSkills.test.ts`「Responses の口も持つホストを
  OpenAI Responses で使うのは食い違いではない」）が同じ形を直接凍結しており、
  P4 の走行はダイアログ操作を含むが警告の報告は無い。実機で踏む価値が
  出るのは免除の表を触るときで、そのときはこのテストが先に赤くなる
- **運用の観察（利用者）**: 「モデルの性能や Memory の書き方だとは思うが、
  **すべて ON にすると web から拾ってきたりなど結果の質がバラける**」。
  どのツールを使うかは**モデルの裁量**で、こちらの制御面は「どれを提示するか」
  まで（`tool_choice` で個別ツールを強制する経路は無い — Required は関数
  ツール込みの全体に掛かる）。**機構は作らない** — 単独 ON という運用が
  そのまま処方で、トグルを 4 本に割った D3 の判断がここで効いている。
  用途の違うテンプレート（金融用 = finance のみ ON / 調査用 = web + fetch）を
  分けて作るのが今の正解
