...
1. **波が列として現れ、セルが個別に確定する**（2026-08-02）— answered と
   interrupted が混在した状態で、セルごとに別々の結末を持った
   （Spec 10 の実機確認と同じ走行）
2. **GUI を再読み込みしても波が残る**（2026-08-04）— アプリは落とさず画面だけを
   作り直し、`useOrchestrator.init()` の `list_plan_waves` 再投影が効くことを確認。
   `波 1・17.4s` と 6 セル（ジェミー + ロボットくん 1〜5 号）が、同じ走行の
   `concordia.log`（`plan bundle: tasks=6 elapsed_ms=17360`）と一致した。
   **記録はコアの in-memory リングに住み、画面はその投影**という設計が、
   画面を捨てて組み直しても成立している

ペイン自身が「1 波（直近 50 波まで・**再起動で消える**）」と名乗っており、
in-memory リングという寿命を画面が正直に語っている（動くふりをさせない）。
...
> 利用者として、村の地図の下で波の進行を眺めたい。なぜなら plan の実行は
