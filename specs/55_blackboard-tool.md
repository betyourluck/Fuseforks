...
- 状態: **rev2（2026-09-17。査読 2 系統 18 点・重複 2 = 実質 16 点 → 採用 13 /
  ...
  **→ P1 完了**（同日。記録は「P1 実装記録」）**→ P2 完了**（同日。記録は「P2 実装記録」）**→ P3 完了**（同日。記録は「P3 実装記録」）**→ P4 完了**（2026-09-18。記録は「P4 実装記録」）**→ P5 完了**（同日。記録は「P5 台帳記録」）**→ P6 完了 = Done**（同日。実機検収 9 件観測 + 1 件は単体テスト。記録は「P6 実機記録」）
...
- **P5 台帳と撤去（完了 2026-09-18。記録は下の「P5 台帳記録」）** — **止血（2026-09-17。条例の「黒板の節を挿入」+ 黒板タブの空表示の案内 =
...
- **P6 実機（完了 2026-09-18。記録は下の「P6 実機記録」）**
...
- ~~**アプリへの登録は P4 へ送った。**~~（**P3 で戻した** — 囲いが入ると、登録が無い版では黒板へ書けない。P3 実装記録）`state.rs` に登録すると、その版から全個体にツールが出て
...
- **囲いは `tools/fs.rs` の 1 実装** — 判定 `is_under_blackboard(work_dir, resolved)`（解決後のパスの、
  作業フォルダからの最初の要素が `blackboard` か）と、拒否の文面 + 計器を持つ
  `BlackboardFence::refuse`。配線は `file` の write / append / mkdir / remove / move（元と宛先）/
  copy（宛先だけ）、`sd` の apply、`yq` の set / remove。どれも**対象を解決した直後**に通す
- **契約へ足した細部**: 大文字小文字は OS を問わず無視（Spec は Windows だけと書いていた。macOS の
  既定も同じフォルダを指す）/ `yq` の書き込み op は **preview の段で断る** / `sd` の preview は通し、
  「`apply: true` では書き込めません」の 1 行を添える（通してから断ると 1 周を失わせる。断っては
  いないので計器は出さない）/ 計器 `blackboard sweep: agent=… removed=N failed=M`
- **D7 の掃除**: `blackboard.rs` の `trash_notes_of`。`read_blackboard_dir` の結果を
  `split_note_name` で割り、前半が id に**完全一致**する付箋を既存の `delete_note`（ごみ箱・名前の
  関門つき）へ渡す。`delete_agent` は world から外す前に `work_dir` を控え、掃除の失敗では止めない
- **アプリへの登録を P4 から P3 へ戻した（P2 の判断の訂正）。** P2 では「黒板タブが表示名で
  持ち主を引く間は全付箋が孤児に見える」を理由に P4 へ送ったが、**囲いが入った版で登録が無いと、
  黒板へ書く経路が 1 本も無くなる**（`file` の write は断られ、文面が指す `blackboard` ツールは
  提示されていない）。孤児のバッジが P4 まで出るほうが軽い。**一般化: Phase をまたいで機構を
  送るときは、間の版をビルドした人に何が起きるかを Phase ごとに数える**（送った時点では正しくても、
  次の Phase が前提を変える）
- **単体 6 本 + 結合 `tests/blackboard_fence_log.rs` 1 本**（ログの宛先は 1 つなので 1 ファイル 1 テスト。
  断った 2 本だけが `blackboard fence:` に残り読み取りでは増えない / `blackboard sweep:` が
  `removed=3` で他の個体の付箋は残る / どの行にも仕事名 = パスが載らない）。
  `file` のテストは**断る 12 通りと通る 3 通りを対で**見る（読み取り・外への copy・囲いの外への write）
- **一発で緑だったので変異 8 回、全部が狙った本で赤**: copy が元も見る / move が宛先だけ見る /
  remove を素通し → `file` の 1 本 / `sd` が preview も断る → `sd` の 1 本 / `yq` が get も断る →
  `yq` の 1 本 / 掃除を前方一致にする（`agent` が `agent_2` を巻き込む）→ 掃除の 1 本 /
...
- **止血の撤去**: `lib/blackboardOrdinance.ts` とそのテスト（17 本）/ `OrdinanceDialog.vue` の「黒板の節を挿入」
  ボタンと `locale` の取り出し / `BlackboardPane.vue` の `checkOrdinance`・案内の枠・`openOrdinance` の emit と
  `readOrdinance` の import / `App.vue` の `@open-ordinance` / 辞書 ja・en の 5 鍵（`blackboard.noOrdinanceSection` /
  `openOrdinance` / `ordinance.insertBlackboard*` 3）。撤去は止血のコミット `2370d97` の差分を逆から数えて回収した
- **初回案内の黒板の歩**: 「条例の中の『黒板の節を挿入』で下書きできます」を「仕事ごとに 1 枚、進行中・保留・完了の列に
  並びます」へ（ja・en）。ボタンが消えたので、残すとラベルを名指しする文が存在しない画面要素を指す
- **DETAIL 日英**: 黒板の節を書き直し — 書き手は `blackboard` ツール / 置き場の前半は `agent_id` / 規約はツールが運ぶ
  （条例が空の村でも書かれる）/ op 6 つ / 囲いと `run` の例外 / 設定のチェック / 削除の掃除 / 見出しの形 / `まとめ.md` の廃止 /
  持ち主は id で引く。止血の段落は消した。同梱ツール表を 10 本（`blackboard` の行）とディレクトリ木に `tools/blackboard.rs`
- **README 3 言語**: 同梱ツールの行に `blackboard` と「書けるのは自分の付箋だけ」。行数は増えていない
- **Spec 54**: 状態の行へ Spec 55 が覆した 4 点、D1 / D2 の見出しへ取り消し線
- **残した記述**: CLAUDE.md の「黒板を KANBAN へ」「黒板の 3 列」の節は日付つきの履歴なので書き換えていない。
  止血の節には撤去済みの 1 行を足した
- vitest 610（止血のテスト 17 本が消えた分）・vue-tsc 0・build 緑
...
**10 件のうち 9 件を実機で観測し、検収 2 は単体テストへ預けた。** 判定は `fuseforks.log` の
`blackboard op:` / `blackboard fence:` / `blackboard sweep:` の 3 行と、ディスクの `blackboard/` で読んだ。

| # | 結果 | 決め手 |
|---|---|---|
| 1 | 合格 | 条例の黒板の節を消した村で `op=list` → `op=write state=doing outcome=ok` → `append` → `move doing→done`。ファイルは `done/agent_10 - Fuseforks X紹介文.md`（前半が id） |
| 2 | **単体テストへ預けた** | 実機で 2 回試して `outcome=exists` が 1 度も出なかった。1 回目は同じ仕事名で書き直さず `append` を選び、2 回目は `read` → **`remove` → `write`** と自分で衝突を避けた。create-only の単体（P2・赤 → 緑）が担保 |
| 3 | 合格 | `op=move state=done to=doing outcome=frozen`。モデルは拒否の文をそのまま報告した（「done の付箋は動かせない・追記もできない」「消してから write し直すか、別の仕事名で」） |
| 4 | 合格 | `blackboard fence: tool=file op=write` が 1 行だけ。同じ走行の `fd` と `file` の read（`body_chars=2064`）は通り、fence 行は増えない |
| 5 | 合格 | 付箋の前半が `agent_10` で、見出しは「イクス - …」、孤児の印なし |
| 6 | 合格 | 設定を外した走行に `blackboard op:` が 0 行。`file` で書こうとして `fence:` が出て、断りの文が「黒板を使わない設定です」へ切り替わった（129 → 92 字）。モデルが挙げた手元の道具の一覧にも `blackboard` が無い |
| 7 | 合格 | 手で作った `イクス - Fuseforks X紹介文.md` に「持ち主が見つからない」、見出しはファイル名のまま。個別のごみ箱で消えた |
| 8 | 合格 | 3 つの状態に 1 枚ずつ持たせて削除 → `blackboard sweep: agent=agent_9 removed=3 failed=0`。3 つのフォルダが空になり、黒板タブに孤児が出ない |
| 9 | 合格 | `blackboard/` の無い `D:\ManualeRAG` で `op=write outcome=ok`、`blackboard\doing\agent_9 - 初回作成テスト.md` がフォルダごと作られた。**その 4 秒前に `file` の write が `fence:` で断られ、同じターンでツールへ切り替えた** |
| 10 | 合格 | 外への copy は `ok=true body_chars=66` で fence 行なし、中への copy は `blackboard fence: tool=file op=copy` と 129 字の断り（検収 4 と同じ文 = 判定は書き込み先の場所で、op ではない） |

**観察（機構は足さない）**:

- **`exists` は実機でほぼ踏まれない。** モデルは `list` / `read` で盤面を見てから書くので、衝突を自分で見つけて
  回り道する。運用としては望ましい動き
- **`done` の凍結は `remove` + `write` の 2 手で回れる。** 意図した「やり直しは別の仕事名で」以外に、消してから
  同じ名前で作り直す経路がある。消し先は OS のごみ箱で `op=remove` がログに残るので隠れた経路ではない。
  拒否の文面自身が 2 つの案内（消してから write / 別の仕事名）を出しているので、仕様どおり
- **旧形式の写しと新形式の本物は「重複」の印が付かない**（ファイル名の完全一致で判定するため）。旧形式は移行期にだけ
  残る形なので網は足さない
- **囲いの断りは 1 周を失うだけで回復する**（検収 9）。歯止めの先に道がある形（#44 の規律）が実機で効いた
- **Rust 側は P4 より前のプロセスのまま検収した**（`version:` 行が 00:16:55 以降に無い。画面は開発サーバーの
  読み直しで P4・P5）。P4 でコアに入れた変更は `まとめ.md` の先頭固定の撤去だけで、どの検収も触れない
