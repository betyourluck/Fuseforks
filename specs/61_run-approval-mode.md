...
- 状態: **P0〜P3 完了（2026-09-27）。残るのは P4 の実機**
...
ステータスバーの統計アイコンの左に、端末アイコン + `select`（3 択）。既定以外の間は発光する —
自動承認は注意色、承認せずに許可は失敗色（記録も残らないので強く知らせる）。色だけに
頼らない（選んでいるモードの名前が字で出ている）。

## 採らなかった形

- **サーヴァントごとの設定**（`run.json` に保存）— 利用者裁定でステータスバーの 1 つへ
- **`world.json` への保存** — 村を配ると設定も配られる
- **3 つをボタンの押し回しで巡る形** — 1 回押すと「自動承認」、2 回で「承認せず」になり、
  押し間違いの代償が大きい。`select` なら選んだものが字で見える
