...
巡る**（承認あり → 自動許可＋承認 → 自動許可 → 承認あり。順番は `lib/runApproval.ts`）。
既定以外の間は発光する — 自動許可＋承認は注意色、自動許可は失敗色（記録も残らないので
...
| `required` / `auto_approve` / `no_approval` | コマンド：承認あり / コマンド：自動許可＋承認 / コマンド：自動許可 |
| 統計の入口 | アイコン + 統計 |
