...
**ID**: 30
**Date**: 2026-08-09
**Status**: **rev1 承認 → P0〜P4 完了。残は P5 実機確認 7 件**（2026-08-09。
コミット `1ae495f` / `c85bdd3` / `4f0d41b` / `5f75d94` / `766ff43` / `5a799a0`。
rust 686 passed・clippy 警告ゼロ・vitest 284 passed・build 緑。
`git grep -i concordia` は **289 → 85 出現**で、85 は全件が意図した記録）。
**村の査読は出さない** —
...
**本 Spec の射程に入れない理由**: リポジトリ名の変更は **repo の中の作業ではない** —
GitHub 側の rename（リダイレクトは張られるが、`git remote` を直さないと
push 先が旧名のまま）、ローカルの作業フォルダのリネーム、CI の設定、
**この村自身の `work_dir`**（サーヴァントは `D:\Github\OutcastsConcordia` を
向いている）が同時に動く。**利用者の裁定と手作業が要る。**

**未決として明記する。** 書かずに放置すると、次に読む人が「追従漏れ」と読んで
直しに来る（S3）。**public の前に決めるべき項目**で、v0.1.0 の窓は
identifier と同じ理由でここにも掛かる。
...
**機械で留めるテストは作らない**（D6）。85 件の除外リストを保守することになり、
**除外リストは必ずもう一度落ちる**。数えるのはこの 1 回だけで、以後は
**この表が基準線**になる。
...
### P5 観測結果（2026-08-09。**残 1 件 = 検収 5**）

| | 結果 |
|---|---|
| **1** | **合格** — `fuseforks.log` が伸び、行頭が `[fuseforks]` |
| **1'** | **合格** — 利用者が確認 |
| **2** | **合格。決め手は `village_id` の更新時刻が `2026-08-08 5:28:27` のまま**（新規生成なら当日になる）。`probe_approvals.json` も 8/08 のまま |
| **3** | **合格。API キーは全件を貼り直した**（利用者 2026-08-09） |
| **4** | **合格** — `.mcp.json` を書き換えて再接続しただけで `ask_fuseforks` が `tools/list` に出た。**Claude Code の再起動は不要**（D3 の「次の接続で自動的に伝わる」が実物で確認された）。封筒は `Neo（外部クライアント）` |
| **5** | **残り。** 入退室通知を起こして System 行の送り手を見る |
| **6** | **合格** — 81 出現、内訳は上の表 |
| **7** | **観測不能**（`no_match`）。`probe_approvals.json` が 18 バイト = ほぼ空で、**前判定つきの予定が 1 件も残っていない** |

**検収 7 が観測不能だったのは、検収の側の誤り。** `schedule probe:` は**前判定つきの
予定が発火したときだけ**出る行で、村にその予定が無ければ**原理的に出ない**。
`Select-String` は一致ゼロだと無音なので、**「観測できない」と「不合格」が
画面上で同じ見た目になる**。**#68 の三例目**（合格条件が存在しない項目を置いた）で、
`village_id` の更新時刻という**常に読める代替の読み口が実在した**のに、
`no_match` という**条件付きでしか現れない読み口**を先に書いていた。
**一般化: 検収の読み口が「ある機能が動いていれば出る」形なら、その機能が
いま村に存在するかを先に数える。**

**D2 の見積もりは正しかった**（`api_key_env` は使っておらず、鍵は全件が資格情報
ストアに居た）。**観測の前に「再登録は実際には要らなかったのでは」と疑ったのは外れ** —
`turn:` が出たことだけを見て、**利用者が既に貼り直したという別の説明を数えて
いなかった**。**成功の観測は、成功させた操作を含めて読む。**
