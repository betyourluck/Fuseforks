...
**ID**: 30
**Date**: 2026-08-09
**Status**: **Done**（2026-08-09。rev1 承認 → P0〜P5 完了。
コミット `1ae495f` / `c85bdd3` / `4f0d41b` / `5f75d94` / `766ff43` / `5a799a0` /
`f02c2ef` / `342533e` / `a213074`。
rust 686 passed・clippy 警告ゼロ・vitest 284 passed・build 緑。
**P5 の実機確認は 7 項目のうち 6 件が合格し、1 件（検収 7）は検収の側の誤りで
観測不能と判定した**。検収 5 は**読み口を書き直してから**観測した — 下記）。
**村の査読は出さない** —
...
| **5**（**rev2 で読み口を差し替え**） | **同じ村の記録に、旧 `【送り手: Concordia】` と新 `【送り手: Fuseforks】` が並ぶ** | `sessions.redb` の逐語（`exports/{session_id}.jsonl` でも同じ）。**起こし方は「1 分ごとの予定を 1 件、旧記録と同じ文面で発火させる」** |
...
**合格条件から「1 つの会話」を外した。** 証明したいのは**書き換えの不在**であって
同居ではない。旧記録がどのセッションに入っているかは合格と無関係で、条件に
残すと**また存在しない状態を待つことになる**（検収 7 と同じ穴）。

**一般化: 検収の読み口に画面を指名する前に、その画面がその値を「保存された
ものとして読んでいるか」「その場で組み立てているか」を数える。** #68 は
*その画面がその値を引いているか*（引いていなければ何も出ない）だったが、
ここは**引いてはいるが毎回作り直している**形で、**画面には必ず何かが出るぶん
質が悪い** — 出た値を見て「合格」と読めてしまう。`Concordia` が出ないことを
「書き換えられた」と誤読する道まで開いていた。

### P5 観測結果（2026-08-09。**全件決着 = Done**）
...
| **5** | **合格**（読み口を差し替えたうえで観測）。`sessions.redb` に `【送り手: Concordia】` と `【送り手: Fuseforks】` が並び、**直後が両方とも `\n【定期実行: 1 分ごと】\n1 と答えて`**。`Concordia` も `Fuseforks` も 9 文字なので**封筒は両方 16 字で揃い、差は 1 語 9 文字だけ**。旧記録は Spec 28 の probe 検証で作った予定のもの |
| **6** | **合格** — 81 出現、内訳は上の表 |
...
**検収 5 の測り方で、計器の性質を 2 つ記録しておく。**

**(a) `sessions.redb` はアプリ稼働中は読めない**（`head` が `Device or resource busy`）。
**しかも `grep` は読めなかったときも一致ゼロを返す** ので、**「無い」と
「読めていない」が同じ 0 として出る**。最初の測定は全パターン 0 で、
**既知の文字列（`ユーザー` / `送り手`）を先に引いて計器を検定していなければ、
旧封筒は存在しないと結論していた**。検収 7 の「観測できないと不合格が
同じ見た目になる」が、画面ではなく計器の側で再演した形。
**一般化: 一致ゼロを根拠にする前に、その計器で必ず一致するものを 1 つ引く。**

**(b) redb への `grep` はレコード数を数えていない。** 同じ封筒の出現数が
30 分で 68 → 59 に**減った**（`送り手` も 224 → 222）。記録は消えていないので、
これは MVCC の解放ページが再利用され、**古いページに残っていたバイト列が
上書きされた**ため。**「在る」の証拠としては強いが、「何件ある」は信用できない** —
この数字を件数として台帳へ書かないこと。

**検収 6 の 81 出現は、本 Spec の外側の数だった。** 2026-08-09 に再測すると
`git grep -i -o concordia` は **173 出現**だが、そのうち **92 が
`specs/30_rename.md` 自身**（D10 の移行手順と本節が旧名を名指しする）で、
**173 − 92 = 81 が上の内訳表と完全に一致する**。基準線は動いていない。
**改名を記録する文書そのものが旧名の最大の在処になる**ので、次に数える人は
**本 Spec を除いた数**と突き合わせること。
