...
> **訂正（2026-08-13）— ハンドルの経路は起票時にも存在していなかった。**
> `TopologyMap.vue` は `#node-default` を上書きしてノードを描いており、その中に
> `<Handle>` を 1 つも置いていない。Vue Flow はスロットを与えるとハンドルごと
> 差し替えるので、**ハンドルは 1 度も描かれていない**。`@connect` のハンドラは
> 在ったが**到達不能**で、`style.css` の `.vue-flow__handle` は死んだ CSS だった。
>
> よって**起票時の経路は 2 つではなく 1 つ**、本 Spec の drop は「3 つ目」ではなく
> **2 つ目**。以下この Spec 内の「ハンドル経路」への言及はすべて同じ誤りを引きずる
> （実測と称した観察は playground の既定ノードでのもので、アプリの挙動ではない）。
>
> **なぜ気づけなかったか**: 画面の案内文（`map.headingHelp`）も DETAIL も同じ
> ことを書いており、**台帳どうしが互いを裏付けていた**。実装を数えた者が
> 一人も居なかった。`failures.md` #53 と同型（契約に「できる」と書いた操作は、
> 入口が実在するか数える）で、今回は**入口どころか描画要素ごと無かった**。
>
> 発覚は 2026-08-13、描画を `v-network-graph` へ差し替えたとき。私が「ハンドル
> 引きが消える」と 3 度書き、利用者が「元からできない」と指摘して実測で確定した。
