...
**ID**: 21
**Date**: 2026-08-06
**Status**: **Done（2026-08-06）**。P0〜P1 + P3 + P4 完了（実機で
カードから絆が張れることを利用者が確認）。**P2 と S4 は利用者判断で削除**
（2026-08-06）— D2 は fps 47.5〜52.9 で基準 60 未達のまま閉じた。理由は
下の D2 の決着。ネイティブ DnD 不発の仮説だけは未実測のまま —
`force-fallback` のコメントは断定形にしない形で維持（P0 実測 7）
...
- **D2 ドラッグ中の視覚フィードバック — PoC の合否基準 3 点を通ったときだけ
  ...
  **決着（2026-08-06 利用者判断）: 再計測せず S4 ごと削除。** fps 基準 3 が
  素朴実装で未達（47.5〜52.9）のまま、最適化して通しにいく価値が無いと
  判断した — ノードは大きく数も少ない（実機の村で 8 体）ので copy カーソルと
  no-op パルスで実用上迷わず、一度 60fps を割った機構を入れるとフレーム予算を
  常時払い続ける。**必要が実測で出てから**（「どこに落ちるか分からない」の
  報告が出てから）別途起票する（頻度を見てから決める、の流儀）
...
- ~~S4 落とせる場所がドラッグ中に分かる~~ — **削除（2026-08-06）**。D2 の
  決着を参照。発見性は copy カーソル（Phase 1）が担う
...
### ~~Phase 2 — 視覚フィードバック~~ — **削除（2026-08-06 利用者判断）**

D2 の決着どおり。代替の `copy` カーソルは Phase 1 で入っている。
再び欲しくなったら（「どこに落ちるか分からない」の報告が出たら）別 Spec で。
