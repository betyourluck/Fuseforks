...
**ID**: 40
**Date**: 2026-08-18
**Status**: **rev5（2026-08-18）— P1 完了 / P2 は Anthropic 側が着地。
D3 は A（重みを変えない）で確定。残るは検収 7（`gpt-5.6-terra`）だけ**
...
- **1.61 倍は入力側だけ。** 実効トークン全体には出力 ×4 も入るので村全体の倍率は
  これより小さく、**書き込みを持つのは Anthropic だけ**なので他ワイヤの個体は
  影響を受けない
- **検収 5 の「予測を観測の前に書く」（#80）を守っていない。** この観測では数値の
  予測を先に書かなかった。**結果が実装の構造から後知恵で説明できてしまう**ので、
  **この 1 件は「予測が当たった」とは言えない**。次の測定（検収 7）では先に書く
- **検収 7 は未了** — P1 以降、OpenAI 互換の口を通ったラウンドが 1 つも記録されて
  いない。`gpt-5.6-terra` の個体を 1 ターン走らせれば閉じる
