...
- **正規化は ~~3 段~~ → 4 段**（**2026-09-20 に (2) を追加**。`failures.md` #128）:
  (1) パス区切り（`/` `\` `:`）を含む `command` は**照合前に拒否**
  （`./python` と `python` を別物として数えると allow が指す対象が曖昧になる）
  **(2) 空白を含む `command` も照合前に拒否** — `command: "git status"` は 1 トークンとして
  `tokens[0]` に入り、`git status *` の先頭 `git` と一致せず `Unknown` に落ちていた。
  `Unknown` は「`allow` へ追加すれば通る」と案内するので、**利用者は通らないパターンを
  足し続ける**。`Malformed` なら `note_pending` に到達しないので輪も切れる。
  **`args` の空白は拒否しない**（逐語で比較する値）
  (3) 比較は**小文字化して行う**（Windows のファイル名は大文字小文字を区別しない）
  (4) Windows では `PATHEXT` の拡張子を**剥がしてから**比較する
...
1. ✅ allow に一致するコマンドが `cwd` で走り、終了コードと出力が返る（2026-08-04、ザリが `curl` を 2 回）
...
11. ~~`allow` が空のとき `run` がモデルへ提示されない~~ —
    **撤回（2026-08-06 利用者裁定）**。`allow` が空でも提示し、
    「何も実行できない」と本文で告げる。提示しないと要求が積めず、
    **承認する対象が生まれない**閉じた輪だった（D10 を参照）
...
- **D10 `run.json` を持たない個体は「全部 pending」。** **これが fail closed** —
  `allow` に一致しない呼び出しは 1 つも走らない。「利用者が許可したものを憶える」
  という思想とも整合する。**初回の手間は欠点ではなく仕様**。
  **「提示されず」の半分は 2026-08-06 の利用者裁定で撤回した** — この D10 は
  **同じ文の中で矛盾していた**（提示しなければ呼び出しは実行フィルタで弾かれ、
  `note_pending` に届かないので「全部 pending」が成立しない）。実際に閉じた輪で、
  **1 件目だけは人が JSON を手で書くしかなかった**。手間は承認ボタンであって
  JSON の手書きではない
