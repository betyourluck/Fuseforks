...
> 利用者として、Gemini のテンプレートに「Google 接地」のチェックを入れる。
...
4. **`toolCall` / `toolResponse` を履歴へ返していない**（未解決）: 現状は
   ...
   **確かめる前に席を作らない**
5. **`groundingMetadata` が実際に来るかは未確認**: 型は書いたが実データを
   見ていない。来れば P4 で本物の URL をプロンプトへ戻せる。来なければ
   「出典は出せない」が確定し、Phase 2 の告知だけが処方として残る。
   当初「原理的に取れない」と断定したのは検証していない推測だった
