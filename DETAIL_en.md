...
Turn on "Review plans" in a servant's settings (**off by default**) and that servant's `plan` stops as a **proposal** instead of fanning out. An edit panel appears at the top of the Work Status tab: fix each task's text, remove tasks, add targets **from the servant's connections**, then choose Dispatch or Discard. This extends "humans draw the ties" to the plan itself — you decide not only who can be asked (the ties) but also what each one is asked this time (the plan).
