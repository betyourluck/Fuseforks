...
**The Blackboard tab is laid out in columns by state** (Spec 54, 2026-09-17), in a fixed order: `In progress → Needs you → Waiting on an agent → On hold → Done`, followed by "No state" (notes directly under `blackboard/`, only when there are any) and "Other: <folder>" (one column per folder outside the five, only when there are any). The five columns show their headers even with zero notes (the view keeps the same shape before and after loading). The root `まとめ.md` is pinned above the columns.
