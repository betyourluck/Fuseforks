...
│       │   ├── tools/blackboard.rs  Built-in tool: blackboard (blackboard notes. a servant writes only its own)
│       │   └── llm/
...
#### Plan Review — an edit window before the fan-out ([Spec 43](specs/43_plan-edit-window.md))

Turn on "Review plans" in a servant's settings (**off by default**) and that servant's `plan` stops as a **proposal** instead of fanning out. An edit panel appears at the top of the Work Status tab: fix each task's text, remove tasks, add targets **from the servant's connections**, then choose Dispatch or Discard. **While a plan is waiting, the "Work Status" tab label turns accent and carries a count badge** (2026-09-17; someone looking at the Blackboard tab cannot see the edit panel, so the tab itself tells them — the same form as the "Command approval" badge in the title bar, never rounded). This extends "humans draw the ties" to the plan itself — you decide not only who can be asked (the ties) but also what each one is asked this time (the plan).
...
| `blackboard` | Read and write blackboard notes (list / read / write / append / move / remove). [Spec 55](specs/55_blackboard-tool.md). Offered whenever there is a work folder (can be turned off with "Write notes on the blackboard") | **Only inside the work folder's `blackboard/`**; a servant writes only its own notes |
...
- **There is a mode that skips approval itself** ([Spec 61](specs/61_run-approval-mode.md)),
  cycled by clicking the status bar button (its label reads "Command: approval" /
  "Command: auto-allow + approve" / "Command: auto-allow"): **approval required** (the default, as above) /
...
The other tab at the center-bottom is the **Blackboard**. It is literally the `blackboard/` folder inside the shared work folder, **written by the agents (with the `blackboard` tool) and by you** ([Spec 55](specs/55_blackboard-tool.md)). **One file per job**, at `blackboard/<state>/<agent_id> - <job>.md`. The first half of the file name is the **servant's id**, not its display name, so renaming a servant does not detach its notes. **There is no path for the GUI to write content**, and nothing is auto-injected into prompts — agents read it when they decide to.

**The tool carries the rules.** The location, the file naming, and the states live in the `blackboard` tool's description and schema, so **notes get written even in a new village whose ordinance says nothing** (until 2026-09-17 the ordinance was the only carrier, and a village with an empty ordinance never got a single note). What stays in the ordinance is practice: read once when starting, return answers as replies, move to done before the final report.

**The `blackboard` tool has six ops** — `list` (the board) / `read` / `write` (creates a new job's note in `doing`; **refuses if that job name already exists**) / `append` (adds to a note in `doing` or `on-hold`) / `move` (changes the state; **a note in `done` does not move** — redo the work under a new job name) / `remove` (to the OS trash). **A servant can write only its own notes.** Job names are normalized (forbidden characters become `_`). One note holds up to 12,000 characters. The tool creates `blackboard/` and the state folders itself.

**`blackboard/` is fenced.** `file` write / append / mkdir / remove / move, `sd` apply, and `yq` set / remove refuse to change anything under `blackboard/` and point to the `blackboard` tool instead (reading, and copying a note out, still work). With a single writer, no note can land outside the naming rules. **Commands allowed through `run` are not blocked** (`run`'s only enclosure is its allowlist). Unchecking "**Write notes on the blackboard**" in a servant's settings removes the tool from that servant (it can still read the board). **Deleting a servant sends its notes to the trash** — ids are reused after deletion, and a new servant with the same id would otherwise read someone else's notes as its own. Notes left in a previous work folder are out of reach.

**The state of a job is its folder** ([Spec 54](specs/54_blackboard-kanban.md)). There are three: `doing` / `on-hold` / `done`, named in ASCII like `blackboard/` itself (the screen translates them). The separator in the file name is the first ` - `; a job name may itself contain ` - `. Notes in any other folder appear in an "Other: <folder>" column.

**A brief is not a blackboard note** (ordinance revised 2026-08-19). A complete document handed to someone else (purpose / inputs / acceptance / where the output goes) is written by the requester with `file` write into `briefs/<request>.md`, and the request message carries that path. The receiver reads the given path instead of searching, and never writes into the brief (progress goes to their own note by append, the answer goes in the reply). **Briefs do not appear in the Blackboard tab** — the tab reads only the files directly under `blackboard/` and one level of state folders.

**The Blackboard tab is laid out in columns by state**, in a fixed order: `In progress → On hold → Done`, followed by "No state" (notes directly under `blackboard/`, only when there are any) and "Other: <folder>" (one column per folder outside the three, only when there are any). The three columns show their headers even with zero notes (the view keeps the same shape before and after loading). A note's heading is "**the current display name - job name**"; hovering shows the id and the file name. The special treatment of `まとめ.md` was retired by Spec 55.

**Whose turn it is shows as a badge** (the three lanes of 2026-09-16 were demoted from columns to badges by Spec 54): **in motion** (the owner is mid-turn or part of an unsettled wave) / **your turn** (the owner's plan is waiting for your review — the same state the Work Status tab shows) / stopped, stopped by a failure, running-idle, or an **orphan** (owner not found / owner on another work folder). The badge is derived on every render from the **owning servant's state**, never from the note's text, so no rule asks agents to write their own status into a note. Column (the job state the servant declared) and badge (the owner state the screen observed) are kept apart on purpose, so a **forgotten move** — "in the In-progress column, but the owner is stopped" — is readable. The owner is looked up by the id in the first half of the file name; a note whose id matches no servant (such as an old-style `<display name>.md`) or whose servant is not facing this work folder gets an orphan badge and is listed first in its column — it can no longer delete its own note. A note whose name also exists in another state gets a **duplicate** badge on both (written rather than moved). **The column-wide clear button exists only on "Done"** (other columns stay). The whole-board eraser and the per-note trash button are unchanged. Clicking a note's header collapses its body (the header stays; the collapsed set is kept on this device and survives switching tabs; a note that moved to another state comes back expanded). The header has an **owner filter** (the servants that have notes, plus "Unknown owner"; the columns keep their shape and only the notes inside shrink; **not saved**, so a restart never opens on a partial board) and **collapse all / expand all** (acts on the visible notes only; if any is expanded it collapses). While filtered, the "Done" clear button removes only the visible notes (the header eraser still clears the whole board).
