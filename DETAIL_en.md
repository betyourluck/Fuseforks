...
**A sticky note grows by `file` append** (ordinance revised 2026-08-19). It is rewritten with write only once, when a new job starts (heading only, old content gone); after that every addition goes to the end — two writers using full-overwrite write on one file lose one of them, append does not destroy what is there. **A brief is not a blackboard note.** A complete document handed to someone else (purpose / inputs / acceptance / where the output goes) is written by the requester with write into `briefs/<request>.md`, and the request message carries that path. The receiver reads the given path instead of searching, and never writes into the brief (progress goes to their own note by append, the answer goes in the reply). **Briefs do not appear in the Blackboard tab** — notes are pinned to the board, briefs are documents on the desk; the tab reads only the flat files directly under `blackboard/`.

**Notes are laid out in three lanes by whose turn it is** (2026-09-16): **In motion** (the owner is mid-turn or part of an unsettled wave) / **Your turn** (the owner's plan is waiting for your review — the same state the Work Status tab shows) / **Hands off** (stopped, failed, running but idle, or an **orphan**). `まとめ.md` is pinned above the lanes. The lane is derived on every render from the **owning servant's state**, never from the note's text, so no rule asks agents to write a status into their note (the ordinance is unchanged). The owner is looked up by the display name in the file name; if that servant is not facing this work folder the note gets an orphan badge — a servant that moved its work folder can no longer delete its own note, so it is listed first as the one you clear by hand. **The lane-wide clear button exists only on "Hands off"** (notes in motion or waiting on you stay). The whole-board eraser and the per-note trash button are unchanged. The three lane headers are shown even with zero notes (the view keeps the same shape before and after loading). Clicking a note's header collapses its body (the header stays; the collapsed set is kept on this device and survives switching tabs).
