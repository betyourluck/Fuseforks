...
The other tab at the center-bottom is the **Blackboard**. It is literally the `blackboard/` folder inside the shared work folder, written by the agents (with the `file` tool) and by you. The way it is used lives in the village ordinance: **one file per job** (`blackboard/<state>/<display name> - <job>.md`, [Spec 54](specs/54_blackboard-kanban.md); until 2026-09-17 it was one file per agent, `blackboard/<display name>.md`, and notes in that shape still show up) as a sticky note, and only the coordinator bundles them into `blackboard/まとめ.md`. **There is no path for the GUI to write content**, and nothing is auto-injected into prompts — agents read it when they decide to. No new mechanism was added: rules plus the existing file tool are the whole implementation.

**The state of a job is declared by its folder** (Spec 54). There are three: `doing` / `on-hold` / `done` (rev2 also had `needs-you` and `waiting`; both were dropped after the first live run — delegation waits inside the turn so there is no moment to declare a wait, and whose turn it is comes from the badges below), named in ASCII like `blackboard/` itself (the screen translates them). A servant starts a job by creating its note under `blackboard/doing/` and **changes the state with `file` move** — the content is untouched, so the append history survives. A move onto an existing name is refused (no overwrite). The separator in the file name is the first ` - `; a job name may itself contain ` - `. Notes in any other folder appear in an "Other: <folder>" column, and folders nested inside a state folder are not read.

**A sticky note grows by `file` append** (ordinance revised 2026-08-19). It is written with write only once, when the job's note is created; after that every addition goes to the end — two writers using full-overwrite write on one file lose one of them, append does not destroy what is there. **A brief is not a blackboard note.** A complete document handed to someone else (purpose / inputs / acceptance / where the output goes) is written by the requester with write into `briefs/<request>.md`, and the request message carries that path. The receiver reads the given path instead of searching, and never writes into the brief (progress goes to their own note by append, the answer goes in the reply). **Briefs do not appear in the Blackboard tab** — notes are pinned to the board, briefs are documents on the desk; the tab reads only the files directly under `blackboard/` and one level of state folders.

**The Blackboard tab is laid out in columns by state** (Spec 54, 2026-09-17), in a fixed order: `In progress → On hold → Done`, followed by "No state" (notes directly under `blackboard/`, only when there are any) and "Other: <folder>" (one column per folder outside the three, only when there are any). The three columns show their headers even with zero notes (the view keeps the same shape before and after loading). The root `まとめ.md` is pinned above the columns.

**The ordinance is the only path that tells servants how to use the blackboard** (2026-09-17). A new village has an empty ordinance and the system prompt never mentions the blackboard, so as things stand no note is ever written. **"Insert blackboard section" in the Ordinance dialog** drafts the rules at the end of the ordinance — the location (`blackboard/<state>/<display name> - <task name>.md`), the state folders, the separator, and `まとめ.md`. Saving is a human action; once inserted the text is yours and the app never touches it again; an English village gets the English text. The wording is assembled from the same constants the screen reads with (`lib/blackboardOrdinance.ts`). **The Blackboard tab shows "The ordinance has no blackboard section" with an "Open ordinance" button only when there are zero notes and the ordinance lacks the spelling `blackboard/doing`** (it says nothing when the ordinance could not be read).

**Whose turn it is shows as a badge** (the three lanes of 2026-09-16 were demoted from columns to badges by Spec 54): **in motion** (the owner is mid-turn or part of an unsettled wave) / **your turn** (the owner's plan is waiting for your review — the same state the Work Status tab shows) / stopped, stopped by a failure, running-idle, or an **orphan** (owner not found / owner on another work folder). The badge is derived on every render from the **owning servant's state**, never from the note's text, so no rule asks agents to write their own status into a note. Column (the job state the servant declared) and badge (the owner state the screen observed) are kept apart on purpose, so a **forgotten move** — "in the In-progress column, but the owner is stopped" — is readable. The owner is looked up by the display name in the file name; a servant not facing this work folder gets an orphan badge and is listed first in its column — it can no longer delete its own note. A note whose name also exists in another state gets a **duplicate** badge on both (written rather than moved). **The column-wide clear button exists only on "Done"** (other columns and `まとめ.md` stay). The whole-board eraser and the per-note trash button are unchanged. Clicking a note's header collapses its body (the header stays; the collapsed set is kept on this device and survives switching tabs; a note that moved to another state comes back expanded).
