 ...
 *
 * 自己接続は張らない。**効くのは drop 経路だけ** — 地図の中でドラッグして
 * 繋ぐ経路は存在しない（2026-08-13 に確定。`failures.md` #102 の一般化 3）。
 * 起票時の doc は「ハンドル経路は自己接続を発火させない」と書いていたが、
 * **その経路自体が無かった**（観察は playground の既定ノードでのもの）。
 * それでも規則側に置くのは、入口が増えたときに検査が付いて回るため。
 */
