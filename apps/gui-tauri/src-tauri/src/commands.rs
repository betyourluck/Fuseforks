...
use agent_core::{
    CommandRegistration, CommandRegistry, CoreError, CoreResult, RagChunk, resolve_program,
};
use tauri::State;
...
/// 画面へ渡す 1 件の登録。**無効の理由も一緒に返す** — 「登録が無い」と
/// 「登録はあるが使えない」は利用者にとっても別の話で、直し方が違う。
#[derive(serde::Serialize)]
#[serde(rename_all = "camelCase")]
...
/// 登録済みコマンドの一覧。
#[tauri::command]
pub async fn list_commands(state: State<'_, AppState>) -> CoreResult<Vec<CommandView>> {
    let registry = state.commands.read().await;
    Ok(registry
        .to_registrations()
        .into_iter()
        .map(|spec| {
            let unavailable = registry
                .get(&spec.name)
                .and_then(|entry| entry.unavailable)
                .map(|reason| reason.reason_ja().to_owned());
            CommandView {
                name: spec.name,
...
/// 登録簿をまるごと保存する。
///
/// **保存のたびに実在検査を掛け直す。** 利用者がパスを直したら、その場で有効に
/// 戻ってほしい（次の起動まで待たせない）。
#[tauri::command]
pub async fn save_commands(
    ...
    let (mut registry, mut notes) = CommandRegistry::from_registrations(commands);
    notes.extend(registry.mark_availability(|program| program.is_file()));

    state.store.save_commands(&registry).await?;
    *state.commands.write().await = registry;
    Ok(notes)
}

/// `PATH` から実行ファイルの絶対パスを引く（登録画面の補助）。
///
/// **記録するのは解決後の絶対パスで、実行時に `PATH` を引き直さない** —
/// どのバイナリが走るかを、その時の `PATH` に委ねない。
#[tauri::command]
pub fn resolve_command_program(name: String) -> Option<String> {
    resolve_program(&name).map(|path| path.display().to_string())
}
...
pub async fn list_command_requests(
    state: State<'_, AppState>,
) -> CoreResult<Vec<CommandRequestView>> {
    Ok(state
        .command_requests
        .read()
        .await
        .requests()
        .iter()
        .map(|request| CommandRequestView {
            name: request.name.clone(),
            attempted_extra_args: request.attempted_extra_args.clone(),
            agent_id: request.agent_id.to_string(),
            first_requested_at_ms: request.first_requested_at_ms,
            count: request.count,
        })
        .collect())
}
...
#[tauri::command]
pub async fn dismiss_command_request(state: State<'_, AppState>, name: String) -> CoreResult<()> {
    let snapshot = {
        let mut log = state.command_requests.write().await;
        log.forget(&name);
        log.clone()
    };
    state.store.save_command_requests(&snapshot).await
}
