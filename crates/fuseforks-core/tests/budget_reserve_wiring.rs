...
fn both_llm_call_sites_reserve_before_calling() {
    ...
    // 予約 1 箇所につき清算は **2 腕**（成功 / 払ったと分かる失敗）。
    // 後者は `failures.md` #103 の処方 — 出力上限で切れた応答は課金されているのに、
    // `?` で抜けると guard の Drop が全額返金していた。2 × 2 = 4 で数える。
    // 3 に落ちたら「どちらかの箇所で失敗の腕を落とした」、5 以上なら
    // 予約していない所で清算している（二重計上）。
    assert_eq!(
        src.matches(".commit(").count(),
        4,
        "予約した 2 箇所とも、成功と払った失敗の両腕で実測清算すること（返しっぱなしにしない）"
    );
}
