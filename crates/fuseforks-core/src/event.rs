...
pub enum CoreEvent {
    ...
    /// なぜ探したのかも読めない。**埋めたのは「なぜ」。**
    ///
    /// **「何を」（引数）と結果の本文は、今もこのイベントでは運ばない**（Spec 57）。
    /// 運ぶのは `call_id` だけで、中身はコアのリング
    /// [`crate::tool_calls::ToolCallStore`] に置き、画面が行を開いたときに引く。
    /// 載せない理由は 3 つ — Tauri への橋は `Lagged` を捨てて続行するので
    /// 取りこぼした本文は復元できない / `broadcast` は購読者ごとに複製する /
    /// 開かれない行が大半。引数と出力には利用者の秘密が入りうるので
    /// （`failures.md` #71）、ログにも `sessions.redb` にも残さない。
    #[serde(rename_all = "camelCase")]
    ToolInvoked {
        /// 実行したエージェント。
        agent_id: AgentId,
        ...
        ///
        /// **発行より前にリングへ記録してある**ので、このイベントを受けた時点で
        /// 必ず引ける。引けなくなるのは押し出し（500 件）と会話の切り替えの後だけ。
        call_id: u64,
    },
    ...
    /// 計画の確認を飛ばすスイッチが切り替わった（Spec 53 D3 — ステータスバー）。
    ///
    /// 状態はコアのメモリだけにあり保存しないので、画面は起動時に 1 回読んだ後は
    /// このイベントで追う（切り替えの入口が 1 つでも、投影の更新を戻り値だけに
    /// 頼らない — 他の画面が同じ状態を読むため）。
    #[serde(rename_all = "camelCase")]
    PlanReviewBypassChanged {
        /// 切り替え後の状態。
        on: bool,
    },
    ...
    ///
    /// 計画の確認のスイッチと同じく、状態はコアのメモリだけにあり、画面は起動時に
    /// 1 回読んだ後はこのイベントで追う。
    #[serde(rename_all = "camelCase")]
    RunApprovalChanged {
        /// 切り替え後のモード。
        mode: crate::command::RunApproval,
    },
