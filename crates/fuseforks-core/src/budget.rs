...
impl BudgetPool {
    ...
    /// 残額（milli 建て）。**打ち切りの計器のためだけにある** —
    /// 「尽きた（残 0）」と「次の 1 呼び出しぶんを確保できない（残 > 0）」を
    /// 区別して記録するため（Spec 38 P4）。
    ///
    /// **門の判定には使わない。** 判定は [`Self::try_reserve`] の CAS が
    /// 行い、ここを読んで分岐すると観測と判定が 2 本に割れる（#105 の形）。
    pub fn remaining_milli(&self) -> u64 {
        self.remaining_milli.load(Ordering::Acquire)
    }
