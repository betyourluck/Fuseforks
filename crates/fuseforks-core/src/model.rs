...
///
/// **読み取り専用の投影**で、ドメインの名詞ではない。`@` を打った瞬間に
/// 1 回だけ取り、補完が開いている間は取り直さない（Spec 24 D6）。
///
/// **種別（`kind`）はここに持たせない。** 将来サーヴァントの `@` 言及を足すとき、
/// その候補は `AgentSnapshot` から**フロントのローカル状態だけで**組める
/// （IPC が要らない）。ここへ `kind` を置くと `"file"` 以外が永久に入らない
/// 空の欄になる — **候補の合流はフロントで起きるので、種別もフロントが持つ**
/// （Spec 24 D2）。
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkDirListing {
    ...
    /// 走査が上限で打ち切られたか。
    ///
    /// **真なら画面に出す**（Spec 24 D4）。候補に出ないファイルがあるのに
    /// 黙っていると「打っても出ないから無い」と読まれる —
    /// この村が `grep` / 広場ログ / `sd` で 3 回間違えて直している形。
    pub truncated: bool,
}
