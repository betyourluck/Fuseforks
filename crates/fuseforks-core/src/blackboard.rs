...
//! 書き手はエージェント（`blackboard` ツール = [`crate::tools::blackboard`]。Spec 55 で
//! `file` / `sd` / `yq` の書き込み系は `blackboard/` の下で拒否される）と人。
//! **GUI からの書き込み経路は作らない** — 「書いてよいのは自分の付箋だけ」を GUI が
//! 迂回する口を開けない。読みも pull のみで、コアはファイル変更を監視しない
...
pub struct BlackboardNote {
    ...
    /// `None` は直下の平置き（「状態なし」）。**閉じた 3 値かどうかは読み手では見ない** —
    /// 3 値を知るのは書き手の `blackboard` ツールだけ（Spec 54 rev3 / Spec 55）。
    /// 3 値の外のフォルダは画面が「その他: <名>」として名指しで出す（閉じた列挙から
