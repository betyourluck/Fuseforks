...
use crate::command::{CommandRegistration, CommandRegistry, CommandRequest, CommandRequestLog};
...
/// 登録済みコマンドのファイル名（Spec 15）。
const COMMANDS_FILE: &str = "commands.json";

/// エージェントが出した登録要求のファイル名（Spec 15）。
///
/// **`commands.json` と分けてある。** 同居させると、登録簿が壊れた JSON に
/// なったときの「空として起動 WARN」で**登録要求まで一緒に消える** —
/// 無人の夜に積んだ要求が、翌朝に人が見る前に落ちる経路を残さない。
const COMMAND_REQUESTS_FILE: &str = "command_requests.json";
...
impl ConfigStore {
    ...
    /// 登録済みコマンドを読み込む。
    ///
    /// **ファイル全体が読めなければ空として返し、理由を `dropped` に載せる。**
    /// `load_schedules` が `Err` を返すのと逆の判断 — あちらは呼び出し側が
    /// 書き戻しを止めるためだが、こちらは**開けない登録簿で起動を止めない**
    /// （閉じた許容なので、空 = 何も実行できない = 安全側へ倒れる）。
    /// 1 件ずつの読み取り失敗は、その 1 件だけを落とす（予定と同じ扱い）。
    pub async fn load_commands(&self) -> LoadedCommands {
        let path = self.root.join(COMMANDS_FILE);
        let text = match tokio::fs::read_to_string(&path).await {
            Ok(text) => text,
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
                return LoadedCommands::default();
            }
            Err(err) => {
                return LoadedCommands::empty_with(format!(
                    "{} を読めませんでした（登録は空として起動します）: {err}",
                    path.display()
                ));
            }
        };

        let file: CommandsFile = match serde_json::from_str(&text) {
            Ok(file) => file,
            Err(err) => {
                return LoadedCommands::empty_with(format!(
                    "{} が JSON として読めません（登録は空として起動します。\
                     直せば次の起動で戻ります）: {err}",
                    path.display()
                ));
            }
        ...
        for row in file.commands {
            match serde_json::from_value::<CommandRegistration>(row.clone()) {
                Ok(spec) => rows.push(spec),
                Err(err) => dropped.push(format!("読めない登録を 1 件落としました: {err}（{row}）")),
            }
        }

        let (registry, mut more) = CommandRegistry::from_registrations(rows);
        dropped.append(&mut more);
        LoadedCommands { registry, dropped }
    }

    /// 登録済みコマンドを保存する。`save_world` と同じく一時ファイル + rename。
    ///
    /// **無効印は書き戻さない** — 別の端末で開いただけで登録が消えるのを避ける。
    pub async fn save_commands(&self, registry: &CommandRegistry) -> CoreResult<()> {
        let file = CommandsFile {
            version: COMMANDS_FILE_VERSION,
            commands: registry
                .to_registrations()
                .into_iter()
        ...
        self.write_atomic(COMMANDS_FILE, &serde_json::to_string_pretty(&file)?)
            .await
    }

    /// 登録要求を読み込む。**壊れていても `commands.json` には影響させない。**
    pub async fn load_command_requests(&self) -> LoadedCommandRequests {
        let path = self.root.join(COMMAND_REQUESTS_FILE);
        let text = match tokio::fs::read_to_string(&path).await {
            Ok(text) => text,
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
                return LoadedCommandRequests::default();
            }
            Err(err) => {
                return LoadedCommandRequests::empty_with(format!(
                    "{} を読めませんでした（登録要求は空として起動します）: {err}",
                    path.display()
                ));
            }
        };

        match serde_json::from_str::<CommandRequestsFile>(&text) {
            Ok(file) => LoadedCommandRequests {
                log: CommandRequestLog::from_requests(file.requests),
                dropped: Vec::new(),
            },
            Err(err) => LoadedCommandRequests::empty_with(format!(
                "{} が JSON として読めません（登録要求は空として起動します）: {err}",
                path.display()
            )),
        }
    }

    /// 登録要求を保存する。一時ファイル + rename。
    ///
    /// 原子性を世界と同格に扱うのは、**壊れたものをどう読むかより、壊れないように
    /// 書くほうが効く**ため（読み側の「空として WARN」は、無人の夜に積んだ要求が
    /// 消える経路をどうしても残す）。
    pub async fn save_command_requests(&self, log: &CommandRequestLog) -> CoreResult<()> {
        let file = CommandRequestsFile {
            version: COMMANDS_FILE_VERSION,
            requests: log.requests().to_vec(),
        };
        self.write_atomic(COMMAND_REQUESTS_FILE, &serde_json::to_string_pretty(&file)?)
            .await
    }
...
/// `commands.json` / `command_requests.json` の形式版数。
const COMMANDS_FILE_VERSION: u32 = 1;
...
struct CommandsFile {
    #[serde(default)]
    version: u32,
    /// 1 件ずつ落とせるように `Value` で受ける（予定と同じ扱い）。
    #[serde(default)]
    commands: Vec<serde_json::Value>,
}

/// `command_requests.json` のワイヤ形。
#[derive(Debug, Serialize, Deserialize)]
struct CommandRequestsFile {
    #[serde(default)]
    version: u32,
    #[serde(default)]
...
/// [`ConfigStore::load_commands`] の結果。
#[derive(Debug, Default)]
pub struct LoadedCommands {
    /// 読めた登録。**まだ実在検査を通していない** — `mark_availability` は
    /// 呼び出し側（`fs` を触れる層）が呼ぶ。
    pub registry: CommandRegistry,
    /// 落とした・丸めたものの説明。空でなければ WARN を出す。
    pub dropped: Vec<String>,
}

impl LoadedCommands {
    fn empty_with(note: String) -> Self {
        Self { registry: CommandRegistry::default(), dropped: vec![note] }
    }
}

/// [`ConfigStore::load_command_requests`] の結果。
#[derive(Debug, Default)]
pub struct LoadedCommandRequests {
    /// 読めた登録要求。
    pub log: CommandRequestLog,
    /// 落としたものの説明。
    pub dropped: Vec<String>,
}

impl LoadedCommandRequests {
    fn empty_with(note: String) -> Self {
        Self { log: CommandRequestLog::default(), dropped: vec![note] }
    }
}
...
mod tests {
    ...
    fn sample_registration(name: &str) -> CommandRegistration {
        CommandRegistration {
            name: name.to_owned(),
            description: "テスト用".to_owned(),
            program: if cfg!(windows) {
                PathBuf::from(r"C:\bin\python.exe")
            } else {
    ...
    #[tokio::test]
    async fn commands_round_trip_through_the_file() {
        let dir = TempDir::new("cmd-roundtrip");
        let store = ConfigStore::new(&dir.0);

        let (registry, dropped) =
            CommandRegistry::from_registrations(vec![sample_registration("test")]);
        assert!(dropped.is_empty(), "{dropped:?}");
        store.save_commands(&registry).await.unwrap();

        let loaded = store.load_commands().await;
        assert!(loaded.dropped.is_empty(), "{:?}", loaded.dropped);
        assert_eq!(loaded.registry.to_registrations(), registry.to_registrations());
    }

    #[tokio::test]
    async fn a_missing_commands_file_is_simply_empty() {
        let dir = TempDir::new("cmd-none");
        let loaded = ConfigStore::new(&dir.0).load_commands().await;
        assert!(loaded.registry.is_empty());
        assert!(loaded.dropped.is_empty(), "無いことは異常ではない: {:?}", loaded.dropped);
    }

    #[tokio::test]
    async fn a_broken_commands_file_starts_empty_and_says_why() {
        let dir = TempDir::new("cmd-broken");
        std::fs::write(dir.0.join(COMMANDS_FILE), "{ これは JSON ではない").unwrap();

        let loaded = ConfigStore::new(&dir.0).load_commands().await;
        assert!(
            loaded.registry.is_empty(),
            "閉じた許容なので、読めない登録簿は空 = 何も実行できない側へ倒す"
        );
        assert_eq!(loaded.dropped.len(), 1, "黙って空にしないこと: {:?}", loaded.dropped);
    }

    #[tokio::test]
    async fn one_unreadable_registration_does_not_take_the_others_down() {
        let dir = TempDir::new("cmd-partial");
        std::fs::write(
            dir.0.join(COMMANDS_FILE),
            r#"{ "version": 1, "commands": [ { "name": "broken" }, { "name": "ok", "program": "/usr/bin/python" } ] }"#,
        )
        .unwrap();

        let loaded = ConfigStore::new(&dir.0).load_commands().await;
        assert_eq!(loaded.registry.len(), 1, "読めた 1 件は開くこと");
        assert!(loaded.registry.get("ok").is_some());
        assert_eq!(loaded.dropped.len(), 1, "落とした 1 件を言うこと");
    }

    #[tokio::test]
    async fn saving_never_writes_back_the_unavailable_mark() {
        let dir = TempDir::new("cmd-mark");
        let store = ConfigStore::new(&dir.0);

        let (mut registry, _) =
            CommandRegistry::from_registrations(vec![sample_registration("test")]);
        // 別の端末で開いた状況（実行ファイルが無い）。
        let notes = registry.mark_availability(|_| false);
        assert_eq!(notes.len(), 1);

        store.save_commands(&registry).await.unwrap();
        let reloaded = ConfigStore::new(&dir.0).load_commands().await;

        assert_eq!(
            reloaded.registry.len(),
            1,
            "無効な登録もファイルに残ること — 消すと、配布した村を別の端末で\
             一度開いて閉じただけで登録が失われる"
        );
        assert!(
            reloaded.registry.get("test").unwrap().unavailable.is_none(),
            "読み直した直後は無印（実在検査は呼び出し側が別に掛ける）"
        );
    }

    #[tokio::test]
    async fn command_requests_round_trip_and_keep_the_first_timestamp() {
        ...
        assert!(log.record("pytest", &args, &id, 100).is_none());
        assert!(log.record("pytest", &args, &id, 999).is_none());
        store.save_command_requests(&log).await.unwrap();

        let loaded = ConfigStore::new(&dir.0).load_command_requests().await;
        assert!(loaded.dropped.is_empty(), "{:?}", loaded.dropped);
        assert_eq!(loaded.log.requests().len(), 1);
        assert_eq!(loaded.log.requests()[0].count, 2);
        assert_eq!(loaded.log.requests()[0].first_requested_at_ms, 100);
        assert_eq!(loaded.log.requests()[0].attempted_extra_args, args);
    }

    #[tokio::test]
    async fn a_broken_requests_file_does_not_take_the_registry_down() {
        let dir = TempDir::new("req-broken");
        let store = ConfigStore::new(&dir.0);

        let (registry, _) = CommandRegistry::from_registrations(vec![sample_registration("test")]);
        store.save_commands(&registry).await.unwrap();
        std::fs::write(dir.0.join(COMMAND_REQUESTS_FILE), "壊れている").unwrap();

        let reopened = ConfigStore::new(&dir.0);
        let requests = reopened.load_command_requests().await;
        let commands = reopened.load_commands().await;

        assert!(requests.log.is_empty());
        assert_eq!(requests.dropped.len(), 1, "黙って空にしないこと");
        assert_eq!(
            commands.registry.len(),
            1,
            "別ファイルにした目的 — 片方が壊れてももう片方を巻き込まない"
        );
        assert!(commands.dropped.is_empty(), "{:?}", commands.dropped);
    }

    #[tokio::test]
    async fn atomic_writes_leave_no_temp_file_behind() {
        let dir = TempDir::new("cmd-temp");
        let store = ConfigStore::new(&dir.0);

        let (registry, _) = CommandRegistry::from_registrations(vec![sample_registration("test")]);
        store.save_commands(&registry).await.unwrap();
        store.save_command_requests(&CommandRequestLog::default()).await.unwrap();

        assert!(!dir.0.join(format!("{COMMANDS_FILE}.tmp")).exists());
        assert!(!dir.0.join(format!("{COMMAND_REQUESTS_FILE}.tmp")).exists());
    }
