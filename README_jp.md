...
| 🗜️ **ツール結果の圧縮** | MCP のツールと `rag` が返した長い本文から、依頼に関係の無い段落をサーヴァントへ渡す前に落とす。判定は**判断専用モデル Jev**。**既定は無効**で、利用者が自分の Cloudflare の鍵を入れるまで**外へは 1 バイトも出ない**。残る部分は逐語、落とした場所には印が入り、`omitted` で読み直せる（[Spec 59](specs/59_jev-tool-result-pruning.md)。送るものは [PRIVACY.md](PRIVACY.md) が正） |
