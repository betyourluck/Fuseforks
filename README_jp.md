...
| 🛠️ **同梱ツール** | `remember` / `grep` / `fd` / `diff` / `sd` / `yq` / `file` / `rag` / `blackboard` / `run`。ファイル系は作業フォルダの外を構造的に読めない（例外は宣言したフォルダを読む `rag` と、囲いが許可リストである `run`。許可リストに無いコマンドの承認は、ステータスバーで「自動承認して許可」「承認せずに許可」へ緩められる — 禁止したコマンドはどのモードでも走らない）。黒板の付箋は `blackboard` でだけ書け、書けるのは自分の付箋だけ |
| 🪧 **ツールの理由** | 道具を使うとき、**何のために使うのか**が 1 行で会話に出る。**モデルの自己申告**であって監査の記録ではない。行を押すと**渡した引数と返ってきた本文**が開き、続いたツール行は 1 行に畳まれる（中身はアプリのメモリにだけ置き、ログにも保存先にも残さない） |
| 🗜️ **ツール結果の圧縮** | MCP のツールが返した長い本文から、依頼に関係の無い段落（JSON なら配列の要素）をサーヴァントへ渡す前に落とす。判定は**判断専用モデル Jev**。**既定は無効**で、利用者が自分の Cloudflare の鍵を入れるまで**外へは 1 バイトも出ない**。残る部分は逐語、落とした場所には印が入り、`omitted` で読み直せる（[Spec 59](specs/59_jev-tool-result-pruning.md)。送るものは [PRIVACY.md](PRIVACY.md) が正） |
| 🎚️ **転送の可否** | 進行役には「会話を引き渡す」道具を持たせない設定。**委譲（訊いて答えを受け取る）と手分けは残る**ので、答えが利用者へ逸れずに戻る |
| 🗣️ **広場ログ** | 他人の会話が聞こえる村。聞かない自由もある（コスト設定として） |
| 📎 **パス補完** | 入力欄で `@` を打つと作業フォルダのファイルが候補に出る。**入るのはパスだけ**で、サーヴァントが探す周回が消える。**`@@` はこの会話でサーヴァントが書いた発話を選び**、その写しを宛先へ渡す — 互いの回答が見えないサーヴァントにも届く |
