...
| 🔌 **MCP** | Paste Claude Desktop's `mcp.json` **as-is**. Shared + per-agent. Both stdio and remote (Streamable HTTP) |
| 🔍 **Grounding** | Gemini's Google Search and URL fetch, Grok's Live Search (web / X), web search on OpenAI and Meta, and Perplexity's web / finance / people search and URL fetch. **The display distinguishes between searched facts, their sources, and facts that went unfound** |
...
| 🛠️ **Built-in Tools** | `remember` / `grep` / `fd` / `diff` / `sd` / `yq` / `file` / `rag` / `blackboard` / `run`. File tools are structurally unable to read outside the work folder (the exceptions are `rag`, which reads declared folders, and `run`, whose enclosure is its allowlist; the status bar can relax approval for unlisted commands to "auto-approve and allow" or "allow without approval" — denied commands never run in any mode). Blackboard notes are written only through `blackboard`, and a servant writes only its own notes |
| 🪧 **Tool Reasons** | When a servant reaches for a tool, **what it is reaching for it for** appears as one line in the conversation. It is **the model's own account**, not an audit record. Click a row to open **the arguments it sent and the text it got back**; runs of tool rows fold into one line (the contents live only in the app's memory — never in the log or the saved conversation) |
| 🗜️ **Tool-result pruning** | Long bodies from MCP tools have paragraphs (or, for JSON, array elements) unrelated to the request dropped before the servant sees them, judged by **Jev**, a judgement-only model. **Off by default** — it needs your own Cloudflare key, and **nothing leaves the machine until one is set**. What stays is verbatim, what was cut is marked, and `omitted` reads the dropped paragraphs back ([Spec 59](specs/59_jev-tool-result-pruning.md); what is sent is spelled out in [PRIVACY_en.md](PRIVACY_en.md)) |
...
| 📎 **Path Completion** | Type `@` to pick a file from the work folder. **Only the path is inserted**, and the rounds a servant spends searching disappear. **`@@` picks a message a servant wrote in this conversation** and hands a copy to the recipient — it reaches servants that cannot see each other's answers |
...
| 🗂️ **Groups** | Section the servant list by task. Each heading toggles visibility and batch start; drag a card into another section to move it. Hiding changes only what you see — delegation and handoff ties are unchanged ([Spec 51](specs/51_agent-groups.md)) |
...
| 📊 **Stats** | What this village has paid, by conversation × **servant × model** × how each turn ended (a servant that switched models gets one row per model). Units are tokens (the same effective-token weights the budget uses). **Register per-model rates and it also shows an approximate cost (`≈ $`)** ([Spec 41](specs/41_model-pricing.md); rates come from a "fetch" button or by hand — the button also fills in the context length ([Spec 50](specs/50_context-length-fetch.md)). **Models without rates drop out of the total, and the screen says so**). Failed turns are counted too. **"All conversations" shows one month at a time, cut at a closing day, and you can page back to earlier months** (the closing day is set under System Settings > Cost Management; the default is end of month, and "All time" is still available — [Spec 42](specs/42_stats-period.md)). **Recording starts with this version** (earlier conversations cannot be counted) |
...
The connection target is OpenAI-compatible / Anthropic / Gemini / xAI / OpenAI / Meta / Perplexity native. **The base URL is flexible**,
