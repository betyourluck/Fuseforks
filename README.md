...
│       │   ├── tools/fs.rs          同梱ツール: grep / diff（作業フォルダ内に限定）
│       │   └── llm/
...
同梱ツールは 3 本。外部の能力は MCP 経由で足す。

| ツール | 何をするか | 範囲 |
|---|---|---|
| `remember` | `Memory.md` へ 1 行追記（自己更新する長期記憶） | 呼び出したエージェントの設定フォルダ |
| `grep` | 正規表現に一致する行を探す（`パス:行番号: 内容`） | **作業フォルダの中だけ** |
| `diff` | 2 ファイルを unified diff で比較 | **作業フォルダの中だけ** |

`grep` / `diff` を同梱するのは、コーディング用エージェントが最も頻繁に使う道具が
この 2 つで、ファイル全文を読むより桁違いに安く速いため（トークン節約は
