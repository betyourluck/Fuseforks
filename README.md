...
| スケジュール実行 | 未実装。エージェントが動き出す契機は**ユーザーの発話しかない**ので、「決まった時刻にやっておく」種類の仕事が表現できない | [Spec 07](specs/07_scheduled-tasks.md)（rev2・査読待ち。契約案は `data_contract.yaml` に記載済み・未凍結）。予定の管理と発火はアプリが持ち、常駐の起動だけ OS に任せる境界に決めた |
