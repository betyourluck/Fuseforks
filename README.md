...
| ターンあたりのトークン予算（[failures.md](failures.md) #41 の処方 3） | 歯止めは全部**回数**建て（ツール 12 回 / 再試行 3 回 / hop 8 回）。1 周ごとに履歴を送り直すので単位コストが増え続け、回数ではコストを縛れない | 歯止めを 1 つコスト建てにする。Anthropic の Task Budgets（claude-sonnet-5 で利用可・最小 20,000）は**モデルへの申告であって強制ではなく**、Gemini 系には無い — 村全体の天井はハーネス側に要る |
