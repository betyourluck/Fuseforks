...
### 保留: エージェントごとの MCP 振り分け

現在の `mcp.json` は**全エージェント共通**で、繋いだサーバーのツールは全員に
提示される。共通の道具（ファイル操作など）はこれで正しい。

一方、**エージェントごとに使えるサーバーを分けたい**要求がある。動機は
「エージェントごとに別の記憶 DB を持たせたい」— 同じ MCP サーバーでも接続先 DB が
違えば、全員に同じものを渡すのは誤りになる。

想定する形は `AgentSpec.mcpServers`（参照する RAG ソースと同型の、名前の集合）。
未指定なら全共通サーバーを引き継ぐのが素直。共通と個別の両方が要るので、
**今の共通設定は残したまま**個別を足す。

未着手。着手時は `data_contract.yaml` の `mcp_contract` を先に更新すること。
...
| エージェントごとの MCP 振り分け | `mcp.json` は全エージェント共通（上記「保留」参照） | `AgentSpec.mcpServers` を足す。`data_contract.yaml` の `mcp_contract` を先に更新 |
