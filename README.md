    ...
    └── gui-tauri/                   ★ 外殻。agent-core に依存する
        ...
        └── src/
            ...
            └── components/
                ...
                ├── PlanWavePane.vue                   中央下段: 波ペイン（plan の実行痕）
...
| 中央上段 | 村の地図 | 常に見る |
| 中央下段 | 波ペイン（plan の実行痕。[Spec 08](specs/08_plan-wave-pane.md)） | 常に見る（仕切りで高さ 80px まで縮められる） |
| 右 | 会話（吹き出し形式） | 常に見る |
...
#### 波ペイン — plan の実行痕（[Spec 08](specs/08_plan-wave-pane.md)）

中央ペインの下段が plan の実行を描く。列 = 波・行 = エージェント・セル =
