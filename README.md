...
### 保留: 同梱ツールに grep / diff を足す

同梱ツールは現在 `remember` の 1 本だけで、外部の能力は MCP へ委ねている。
ただし **`grep` と `diff` は早めに同梱したい**。理由は、コーディング用エージェントが
最も頻繁に使う道具がこの 2 つで、ファイル全文を読むより桁違いに安く速いから
（トークン節約はこの製品の最重要課題の一つ）。

MCP の filesystem サーバーにも読み取り・検索はあるが、外部プロセスへ依存せずに
使えることに意味がある（インストール不要で誰の環境でも動く）。

置き場は `crates/agent-core/src/tools/`。`AgentTool` を実装するだけで
オーケストレーター側の変更は要らない。検討点:
- 探索範囲をどう縛るか（ワークスペース内に限るか、任意パスを許すか）
- 出力の上限（巨大な一致でプロンプトを埋めない）
- `diff` の対象（ファイル同士か、設定ファイルの版間か）
