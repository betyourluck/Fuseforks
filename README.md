...
| 会話の永続化とセッション管理（利用者要望 2026-07-30 / 2026-08-02） | **設計は凍結済み・実装はこれから**（[Spec 12](specs/12_session-persistence.md)）。いまは会話ログもエージェントの履歴もプロセス寿命で、閉じると消える | `{workspace}/sessions.redb` の 1 ファイルに保存し、複数の会話を持って切り替える。**起票時の仮説「履歴は殺す側」は否定された** — この村には履歴が 2 層あり（村の会話ログ / エージェント個別のプロンプト履歴）、**片方から他方を復元できない**。会話ログだけ保存すると画面は正しいのに全員が健忘症で始まる |
