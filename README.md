...
| 波の実行ビュー（Airflow の Graph/Grid 相当・利用者要望 2026-07-30） | 観測は stderr のみ（`plan wave:` / `plan bundle:` に wave 番号・宛先・所要時間が既に載る）。描画基盤の vue-flow は `TopologyMap` で稼働中 | `plan_id` + `CoreEvent`（波の開始/完了）を足して GUI ペインへ。Spec 04 Notes 12 の `plan_id` トリガーはこれが 2 つ目の発火条件 |
