...
| 📊 **Stats** | What this village has paid, by conversation × **servant × model** × how each turn ended (a servant that switched models gets one row per model). Units are tokens (the same effective-token weights the budget uses), **never converted to currency**. Failed turns are counted too. **Recording starts with this version** (earlier conversations cannot be counted) |
