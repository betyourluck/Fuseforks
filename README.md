...
**前の会話は捨てられずディスクに残る**（Spec 12）— ただし戻るための画面は
まだ無い（P3）。
...
| 会話の永続化とセッション管理（利用者要望 2026-07-30 / 2026-08-02） | **保存と再開は動いている・切り替えの画面がまだ無い**（[Spec 12](specs/12_session-persistence.md) の P0〜P2 完了）。閉じて開き直すと前回の会話が戻り、エージェントも前の話を踏まえて答える。「新規チャット」は捨てずに新しい会話を開く。ただし**会話の一覧・切り替え・分岐・削除の UI は未実装**（P3）で、いまは常に最後に使った会話が開く。要約して続ける機能も未実装（P4）。**会話はディスクに平文で残る** | `{workspace}/sessions.redb` の 1 ファイルに保存し、複数の会話を持って切り替える。**起票時の仮説「履歴は殺す側」は否定された** — この村には履歴が 2 層あり（村の会話ログ / エージェント個別のプロンプト履歴）、**片方から他方を復元できない**。会話ログだけ保存すると画面は正しいのに全員が健忘症で始まる |
