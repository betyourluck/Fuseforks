...
API キーの実値はモデルテンプレートに保持できない。持てるのは**環境変数名だけ**で、
実値はプロセスの環境から解決する。設定ファイル（`world.json`）は平文で保存されるため、
`^[A-Za-z_][A-Za-z0-9_]*$` の書式検査で実値の混入を拒否する。強制点は 3 箇所:

1. `World::upsert_template` — 保存時に拒否する
2. `World::from_persisted` — 読み込み時に不正値を落とす（塞ぐ前に書かれた分の回収）
3. `Orchestrator::bootstrap` — 正規化で内容が変わったら起動時にファイルへ書き戻す

3 が要るのは、入口だけ塞いでも既に漏れたファイルは次の編集操作まで残るため。

> この欄は当初ただの `String` で、防御は UI のラベルと注意書きだけだった。
> 運用初日に実キーが貼られて平文で保存された。**注意書きは制御ではない。**
...
応答本文にも理由（欠けている環境変数名など）が入る。退避したバックエンドは
キャッシュしないので、原因を直せばそのまま復帰する。

> 「エコー応答」とだけ名乗る実装だった頃、環境変数がプロセスへ渡っていないだけの
> 状態で偽の応答が返り続け、原因に辿り着けなかった。**退避は許すが、黙って退避しない。**

### Windows で環境変数が効かないとき

`setx` やシステムのプロパティで設定した環境変数は、**既に起動しているプロセスには
反映されない**。`tauri dev` を動かしたまま変数を足しても効かないので、
ターミナルごと閉じて、新しいターミナルから起動し直すこと。

```bash
cd apps/gui-tauri && bun run tauri dev
```

実際に LLM へ繋ぐには、⚙ からモデルテンプレートを作り、`API キーの環境変数名` に
変数名（例 `ANTHROPIC_API_KEY`）を入れる。**キーの実値を入れると保存が拒否される。**
実値は OS の環境変数に設定し、その状態でアプリを起動すること。
