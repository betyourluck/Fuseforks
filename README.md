...
| コマンド実行ツール（利用者要望 2026-07-31） | 同梱ツールはファイル操作まで（Spec 09）。任意コマンドは持たない | **危険コマンドの関門（Hook）とセットでなければ着手しない** — 実行そのものより、何を止めるかの設計が本体。作業フォルダ境界はコマンドには効かない（`cd` で出られる）ので、囲いの考え方から作り直しが要る |
