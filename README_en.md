...
**The previous conversation is kept on disk rather than discarded** (Spec 12). The adjacent "Conversations" button reopens it, and also offers **forking** (duplicating a conversation up to a chosen request), **exporting** to JSONL, and **deleting**. Reopening and forking are refused while a turn is in flight, because the answer would land in a different conversation — stop it first with "■ Stop".
...
| Conversation persistence and session management (user requests, 2026-07-30 / 2026-08-02) | **Everything but summarization is implemented; not yet verified on the real app** ([Spec 12](specs/12_session-persistence.md), phases P0–P3 done). Close and reopen the app and the previous conversation comes back, with agents answering in light of what was said before. "Conversations" in the chat pane header lists saved conversations: reopen, fork, export, delete. "New chat" now opens a new conversation instead of discarding the current one. **Summarize-and-continue is not implemented yet** (P4). **Conversations are stored on disk in plain text.** | Store everything in a single `{workspace}/sessions.redb`, keep several conversations, and switch between them. **The original hypothesis "histories are candidates to die" was refuted**: this village has two layers of history (the village conversation log and each agent's own prompt history), and **neither can be reconstructed from the other**. Persisting only the conversation log yields a screen that looks correct while every agent starts amnesiac. |
