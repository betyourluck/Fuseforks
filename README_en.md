...
**The previous conversation is kept on disk rather than discarded** (Spec 12) — though the UI for returning to it does not exist yet (P3).
...
| Conversation persistence and session management (user requests, 2026-07-30 / 2026-08-02) | **Saving and resuming work; the switching UI does not exist yet** ([Spec 12](specs/12_session-persistence.md), phases P0–P2 done). Close and reopen the app and the previous conversation comes back, with agents answering in light of what was said before. "New chat" now opens a new conversation instead of discarding the current one. But the UI for listing, switching, forking and deleting conversations is **not implemented yet** (P3), so the most recently used conversation is always the one that opens. Summarize-and-continue is also not implemented (P4). **Conversations are stored on disk in plain text.** | Store everything in a single `{workspace}/sessions.redb`, keep several conversations, and switch between them. **The original hypothesis "histories are candidates to die" was refuted**: this village has two layers of history (the village conversation log and each agent's own prompt history), and **neither can be reconstructed from the other**. Persisting only the conversation log yields a screen that looks correct while every agent starts amnesiac. |
