...
| 右 | 会話（吹き出し形式）。入力欄の下に**表示を消す**ボタン（**表示だけ。会話は残る**）と、その左に**コンテキスト使用率の輪**（[Spec 49](specs/49_context-usage-ring.md)。選択中の個体の**直近の呼び出し 1 回**の入力 ÷ テンプレートの**コンテキスト長**。75% 以上で黄、90% 以上で赤。**ターンの最中は動かず、確定後 1 秒以内に追従**。分母は手入力なので、100% を超えた数字が出たらテンプレートのコンテキスト長が実際の窓より小さい。再起動後は最初のターンまで出ない） | 常に見る |
