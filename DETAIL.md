...
束ねる。**GUI から内容を書く経路は存在しない**し、モデルへの自動注入もしない —
エージェントは読みたいときに自分で読む。仕組みを増やさず、規則と既存の
ファイルツールだけで成立させている。

**付箋は `file` の append で育てる**（2026-08-19 の条例改訂）。write で書き直すのは
新しい仕事に着手した 1 回だけ（見出しだけにして古い内容を消す）で、以後は末尾へ足す —
...
画面が読むのは `blackboard/` 直下の平置きだけ。

**付箋は「持ち主の手番」で 3 列に並ぶ**（2026-09-16）— **動いている**（持ち主がターンの最中か、
未確定の波に参加している）/ **あなたの手番**（持ち主の計画が確認待ち。作業状況タブと同じ状態）/
**手が離れている**（停止中・失敗・起動しているが待機・**孤児**）。`まとめ.md` は列の外で先頭に固定。
列は付箋の中身からではなく**持ち主のサーヴァントの状態から画面が毎回引く**ので、付箋に状態を
書かせる規則は増えていない（条例はそのまま）。持ち主はファイル名の表示名で引き、その個体が
この作業フォルダを向いていなければ孤児のバッジが付く — 作業フォルダを移した個体の付箋は本人が
消せないので、人が消す候補として列の先頭に来る。**一括の消し口は「手が離れている」列にだけ付く**
（動いている / あなたの手番の付箋は残る）。全部消す消しゴムと 1 枚ずつのごみ箱は従来どおり。
付箋が 0 枚でも 3 列の見出しは出る（読み込みの前後で画面の形を変えない）。付箋は見出し行を
押すと本文を畳める（見出し行は残る。畳んだ状態は端末に保存され、タブを切り替えても戻る）。
