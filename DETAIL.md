...
**1 仕事 1 ファイル**（`blackboard/<状態>/<表示名> - <仕事名>.md`。
[Spec 54](specs/54_blackboard-kanban.md)。2026-09-17 までは 1 人 1 ファイル
`blackboard/<表示名>.md` で、その形の付箋もそのまま読める）の付箋形で、進行役だけが
`blackboard/まとめ.md` に束ねる。**GUI から内容を書く経路は存在しない**し、モデルへの自動注入も
しない — エージェントは読みたいときに自分で読む。仕組みを増やさず、規則と既存の
ファイルツールだけで成立させている。

**仕事の状態はフォルダで宣言する**（Spec 54）。状態は `doing`（進行中）/ `on-hold`（保留）/
`done`（完了）の 3 つで（rev2 の「あなたの判断待ち」「エージェント待ち」は実機で使われず落とした —
委譲はターンの中で待つので宣言する時点が無く、人の手番は下のバッジが機構で出す）、名前は `blackboard/` と同じく言語に依存しない英字（画面の表示は辞書が訳す）。新しい仕事に
着手したら `blackboard/doing/` に付箋を作り、**状態が変わったら `file` の move でフォルダを
移す** — 中身に触れないので append の履歴がそのまま残る。宛先に同じ名前があれば move は
拒否される（上書きしない）。ファイル名の区切りは最初の ` - ` で、仕事名の中に ` - ` があっても
よい。3 つ以外のフォルダに置いた付箋は「その他: <フォルダ名>」の列に出る。状態フォルダの中に
さらにフォルダを作っても画面は読まない。

**付箋は `file` の append で育てる**（2026-08-19 の条例改訂）。write で書くのは
新しい仕事の付箋を作る 1 回だけで、以後は末尾へ足す —
全文上書きの write を 2 人が同じファイルに使うと片方が消えるが、append は既存を壊さない。
**指示書は黒板ではない。** 相手に渡す完結した文書（目的 / 入力 / 合格基準 / 出力先）は
`briefs/<依頼名>.md` に依頼を出す側が write で書き、依頼文にそのパスを書く。受け手は
探さず書かれたパスを read し、指示書には書かない（進捗は自分の付箋へ append、答えは返信）。
**黒板タブに指示書は出ない** — 付箋は板に貼るもの、指示書は机の上の文書、という分け方で、
画面が読むのは `blackboard/` の直下と、状態フォルダ 1 段の中だけ。

**黒板タブは状態ごとの列に並ぶ**（Spec 54・2026-09-17）— 列の並びは `進行中 → 保留 → 完了` で固定、
その下に「状態なし」（`blackboard/` 直下の付箋。あるときだけ）と「その他: <フォルダ名>」
（3 つ以外のフォルダ。フォルダごとに 1 列・あるときだけ）。
3 つの列は付箋が 0 枚でも見出しを出す（読み込みの前後で画面の形を変えない）。直下の `まとめ.md` は
列の外で最上部に固定。

**黒板の使い方をサーヴァントへ伝える経路は条例だけ**（2026-09-17）。新しい村の条例は空で、
システムプロンプトは黒板に触れないので、そのままでは付箋は 1 枚も書かれない。**条例ダイアログの
「黒板の節を挿入」**が、置き場（`blackboard/<状態>/<表示名> - <仕事名>.md`）・状態のフォルダ・
区切り・`まとめ.md` の決まりを条例の末尾へ下書きする（保存は人が押す。挿入後は利用者の文章で、
アプリは以後触らない。英語の村には英語で入る）。文面は画面の読み手と同じ定数から組む
（`lib/blackboardOrdinance.ts`）。**黒板タブは、付箋が 0 枚で条例に `blackboard/doing` の綴りが
無いときだけ「条例に黒板の節がありません」と「条例を開く」を出す**（条例を読めなかったときは
何も言わない）。
...
ような**動かし忘れ**が読める。持ち主はファイル名の表示名で引き、その個体がこの作業フォルダを
向いていなければ孤児のバッジが付き、列の中で先頭に来る — 作業フォルダを移した個体の付箋は本人が
消せない。同じ名前の付箋が別の状態にもあると**重複**のバッジが両方に付く（move ではなく write で
作られた形）。**一括の消し口は「完了」列にだけ付く**（他の列と `まとめ.md` は残る）。全部消す
...
個体と片端でも掛かる辺が消え（要約行に「非表示: サーヴァント N / 絆 M」が出る）、
