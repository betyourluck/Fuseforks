    ...
    └── gui-tauri/                   ★ 外殻。fuseforks-core に依存する
        ...
        └── src/
            ...
            └── components/
                ...
                ├── StatusBar.vue                      最下段: MCP サーバーの待ち受け・**計画の確認を飛ばすスイッチ**・**統計への入口**・日付と時刻（診断ログと同じ形式）・版番号
...
| 最下段 | ステータスバー（**MCP サーバーの待ち受け**・**計画の確認を飛ばすスイッチ**（[Spec 53](specs/53_unattended-plan-and-verifier.md)。オンの間は発光して「確認なし」と出る。保存せず再起動でオフ）・日付と時刻・版番号） | 常に見る（22px の帯） |
...
- **無人で回すときは窓を飛ばせる**（[Spec 53](specs/53_unattended-plan-and-verifier.md)・2026-09-15）。
  ...
  - **ステータスバーのスイッチ** — オンの間は利用者の依頼でも窓を開けない。**保存しない**
    （付けっぱなしで再起動すると、確認が要る村で黙って確認が消え続けるため。再起動をまたいで
    無人で回したいものは予定の側で指定する）
