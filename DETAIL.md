...
有効にすると、**MCP のツールと `rag`** が返した本文のうち、そのターンの依頼に
...
`McpTool` と `RagTool` の 2 つだけで、**名前の除外リストで持たない** — 新しい
同梱ツールは何もしなければ対象外になる。`file` は `sd` の材料で逐語が要り、
`run` の固定枠 12,000 字は繰り返しの検出（同じ結果の完全一致）のためにある。
