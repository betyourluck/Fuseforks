    ...
    └── gui-tauri/                   ★ 外殻。fuseforks-core に依存する
        ...
        └── src/
            ...
            └── components/
                ...
                ├── FirstRunTour.vue                   初回起動の案内（9 歩。空の村でだけ出る。設定 ＞ 全般 ＞ 案内で呼び直し）
                ...
                ├── StatusBar.vue                      最下段: MCP サーバーの待ち受け・**統計への入口**・日付と時刻（診断ログと同じ形式）・版番号
...
**旧い版のバイナリで村を開くと、グループと所属は未知の欄として消える**
（[failures.md](failures.md) #112 の形）。
...
| ページ | 中身 |
|---|---|
| 全般 | **ユーザー**（自分の呼び名とアイコン）、言語（日本語 / English）、**案内**（初回起動に出る 9 歩の手順をもう一度見る）。言語は初回だけ OS の設定から推定し、以後は再判定しない |
...
| ユーザーインターフェース | **テーマ**（ダーク / ライト）、**会話の文字**（会話ペインの表示倍率。90〜200% の 7 段、既定 100%。**端末に保存・選んだ瞬間に反映**）、メッセージ表示/非表示。最後は 3 つ — 「絆を切る」の確認、「閉じる前」の確認、**入退室の通知を会話ペインに出すか** |
