...
| 最下段 | ステータスバー（**MCP サーバーの待ち受け**・**計画の確認を飛ばすスイッチ**（[Spec 53](specs/53_unattended-plan-and-verifier.md)。オンの間は発光して「確認なし」と出る）・**コマンドの承認モード**（[Spec 61](specs/61_run-approval-mode.md)。承認が必要 / 自動承認して許可 / 承認せずに許可。既定以外の間は発光する）。**2 つのスイッチはこの端末に記憶され、次の起動でも同じ状態で始まる**・日付と時刻・版番号） | 常に見る（22px の帯） |
...
- **承認そのものを省くモードはある**（[Spec 61](specs/61_run-approval-mode.md)）。
  ステータスバーで 3 つから選ぶ — **承認が必要**（既定・上のとおり）/
