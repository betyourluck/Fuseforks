...
| 🗜️ **工具结果的压缩** | 从 MCP 工具与 `rag` 返回的长正文中，在交给仆从之前删去与委托无关的段落。判定交由**只作判断的模型 Jev**。**预设为停用**，在使用者填入自己的 Cloudflare 金钥之前**不会有任何一个位元组送到外部**。保留的部分为逐字原文，删去之处会留下记号，并可用 `omitted` 重新读取（[Spec 59](specs/59_jev-tool-result-pruning.md)。送出的内容以 [PRIVACY.md](PRIVACY.md) 为准） |
