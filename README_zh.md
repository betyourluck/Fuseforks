...
| 🔍 **接地（Grounding）** | Gemini 的 Google 搜索、Grok 的 Live Search（web / X）、OpenAI 与 Meta 的 web 搜索。**明确区分显示搜索到的事实与未返回来源的事实** |
...
连接端是 OpenAI 兼容 / Anthropic / Gemini / xAI / OpenAI / Meta 的原生方式。**base URL 是自由的**，因此
