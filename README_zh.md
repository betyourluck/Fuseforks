...
| 🛠️ **自带工具** | `remember` / `grep` / `fd` / `diff` / `sd` / `yq` / `file` / `rag` / `blackboard` / `run`。文件类工具无法结构化地读取工作文件夹之外的内容（例外是读取已声明文件夹的 `rag`，以及围栏为允许列表的 `run`；允许列表之外的命令的批准可在状态栏放宽为“自动批准并允许”或“不批准直接允许”——被禁止的命令在任何模式下都不会运行）。黑板便签只能通过 `blackboard` 写入，且只能写自己的便签 |
| 🪧 **工具的用途** | 使用工具时，**出于何种目的使用**会以一行显示在对话中。是**模型的自报**，不是审计记录。点击该行可展开**传入的参数和返回的正文**，连续的工具行会折叠成一行（内容只保存在应用的内存中，不写入日志或已保存的对话） |
| 🗜️ **工具结果的压缩** | 从 MCP 工具返回的长正文中，在交给仆从之前删去与委托无关的段落（JSON 则为阵列的元素）。判定交由**只作判断的模型 Jev**。**预设为停用**，在使用者填入自己的 Cloudflare 金钥之前**不会有任何一个位元组送到外部**。保留的部分为逐字原文，删去之处会留下记号，并可用 `omitted` 重新读取（[Spec 59](specs/59_jev-tool-result-pruning.md)。送出的内容以 [PRIVACY.md](PRIVACY.md) 为准） |
| 🎚️ **转移的可行性** | 对主持者设定为不持有「移交对话」的工具。**委派（询问并接收答复）与分派仍保留**，因此答复不会偏离而返回给使用者 |
| 🗣️ **广场日志** | 能听到他人对话的村庄。也有不听的自由（作为成本设置） |
| 📎 **路径补全** | 在输入栏输入 `@` 时，工作文件夹中的文件会作为候选项出现。**进入的只有路径**，消除了仆从寻找的循环。**`@@` 选取仆从在本次对话中写下的发言**，并将其副本交给收件方 — 即使是看不到彼此回答的仆从也能收到 |
